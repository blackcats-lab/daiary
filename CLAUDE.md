...
| **Phase 1 Sprint 2: AI 生成機能** | 🚧 進行中（#7-#8 ハッシュタグ生成・ai_tags 保存 完了。次: caption / 一覧でのタグ表示） | Week 5-6 |
...
実装の記録は Notion の「📝 実装記録」ページ（dAIary プロジェクト概要の子ページ、ID: `357367739cf781cd834ec07171d4e34c`）に蓄積している。**Phase / Sprint の区切り**で更新する。

### 更新タイミング

以下のいずれかに該当したら追記する:

- **Sprint の完了**（例: Sprint 1 の全タスクが develop にマージされた）
- **Phase の完了**（例: Phase 1 の全 Sprint 完了）
- **節目の機能リリース**（PR が複数本まとまった大きな機能の完成）

「PR 1 本ごと」では更新しない（粒度が細かすぎる）。

### 更新手順

1. **対象 PR の範囲を確認**
   - 直近の Notion 記録に書かれている最後の PR を確認（ページ内の見出し `## PR #N` の最大値）
   - その次の PR から最新マージ分までが追記対象

2. **PR 情報の収集**
   - `gh pr list --state merged --limit 30 --json number,title,mergedAt,headRefName --jq 'sort_by(.number) | .[] | "#\(.number)\t\(.mergedAt[0:10])\t\(.headRefName)\t\(.title)"'`
   - 詳細なコミットは `git log <prev-merge>..<latest-merge> --oneline`
   - PR 本文も参考にする: `gh pr view <number>`

3. **Notion ページに追記**
   - MCP の `notion-fetch` で現在のページ内容を取得
   - `notion-update-page` で既存内容の末尾に新規 PR セクションを追加（**既存セクションは上書きしない**）
   - 各 PR は次のフォーマットで記載する:

     ```markdown
     ## PR #N — <タイトル>

     **マージ日**: YYYY-MM-DD / **base**: develop / **Sprint X #Y-#Z**（該当する場合）
     ...
     ### 主な変更
     - コミット単位で箇条書き
     - 新規ファイル / 重要な API 変更 / 設計判断を明記

     ### 設計メモ（該当する場合）
     - 既知の制約 / 想定リスク / 後続 PR への申し送り
     ```

4. **進捗サマリも併せて更新**
   - ページ冒頭の「進捗サマリ」テーブルで Sprint の状態を最新化（✅ 完了 / 🚧 進行中 / ⏳ 未着手）

### 記録に含めるべき情報

PR の表面的な内容だけでなく、**後で「なぜこの実装になったのか」を辿れる情報**を残す:

- **設計判断の理由**（A/B/C のうち B を選んだ理由など）
- **ハマった点と解決策**（実装中に発見した SDK の制約・既知バグなど）
- **次 PR への申し送り**（スコープ外として明示的に外したもの）
- **実機検証で確認した挙動**（特にプラットフォーム依存の部分）

メモリに保存した知見（`memory/MEMORY.md` 経由）と整合させる。

### 注意点

- Notion ページは **追記のみ**。過去の記録は削除しない（履歴として残す）
- Notion ID は MCP の URL から拾う: `https://www.notion.so/...XXXX` の `XXXX` 部分
- ページの URL を変更したり、別の場所に移動したりしないこと（CLAUDE.md からの参照が壊れる）
